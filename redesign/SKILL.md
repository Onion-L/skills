...
**If the session ID is not provided:**
- Look for `directions.json` files under `gallery/` (or the output root used by `design-gallery`).
- If exactly one session exists, use it automatically.
- If multiple sessions exist, list them and ask the user to pick one:
